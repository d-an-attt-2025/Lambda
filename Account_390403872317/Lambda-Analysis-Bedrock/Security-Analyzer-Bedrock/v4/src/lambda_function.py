...
def fetch_logs_around_time(event_time_str: str) -> str:
    ...
    # Option 1 (khuyến nghị): lấy log "trước alarm" để giảm nhiễu
    start_time = center_ms - window_ms
    end_time = center_ms

    # Nếu bạn muốn đối xứng trước/sau, dùng option 2:
    # half = window_ms // 2
    # start_time = center_ms - half
    # end_time = center_ms + half
...
""".strip()
    ...
    try:
        ...
        # cố gắng bóc JSON nếu model lỡ in thêm text thừa
        l = raw_output.find("{")
        r = raw_output.rfind("}")
        if l != -1 and r != -1 and r > l:
            raw_output = raw_output[l:r+1]

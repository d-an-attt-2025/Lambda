...
def parse_waf_log(msg: str):
    """
    Parse JSON WAF log để lấy:ip, user_agent, geo_region = parse_waf_log(msg)
    - httpRequest.ip, user_agent, geo_region = parse_waf_log(msg)
    ...
    action = data.get("action")  # "ALLOW" hoặc "BLOCK"
    return ip, user_agent, geo_region, action
...
def lambda_handler(event, context):
    ...
    if "awslogs" not in event:
        ...
        ip, user_agent, geo_region, action = parse_waf_log(msg)

        # CHỈ ĐẾM ALLOW: không phải ALLOW thì bỏ qua luôn
        if action != "ALLOW":
            print(f"[MANUAL] Skip non-ALLOW action={action}")
            return {"status": "skip_non_allow", "action": action}
        ...
        window = int((event_ts_ms / 1000) / TTL_COUNTER) * TTL_COUNTER
        pk = f"COUNT#{ip}"
        sk = f"WINDOW#{window}"
        expire = int(time.time()) + TTL_COUNTER
    ...
    for e in events:
        ...
        # CHỈ ĐẾM ALLOW
        if action != "ALLOW":
            print(f"[SUB] Skip non-ALLOW action={action}")
            continue
